...
from
    store_sales_tcpds ss join date_dim_tcpds d1 on d1.d_date_sk = ss.ss_sold_date_sk
    join store_tcpds s on s.s_store_sk  = ss.ss_store_sk
 where
 ...
 and s.s_state in
             ( select s_state
               from  (select s_state as s_state, sum(ss_net_profit),
                      ...
                      from   store_sales_tcpds store_sales, store_tcpds store, date_dim_tcpds date_dim
